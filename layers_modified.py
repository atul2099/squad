...
class Embedding(nn.Module):
    ...
    def __init__(self, word_vectors, char_vectors, hidden_size, drop_prob, char_channel_size, char_channel_width):
        ...
        self.word_embed = nn.Embedding.from_pretrained(word_vectors, freeze = False)
        self.char_embed = nn.Embedding.from_pretrained(char_vectors, freeze = False)

        self.char_conv = nn.Conv2d(1, char_channel_size, (char_vectors.size(1), char_channel_width))

        self.proj = nn.Linear(word_vectors.size(1)+char_channel_size, hidden_size, bias=False)
        self.hwy = HighwayEncoder(2, hidden_size)

    def forward(self, x1, x2):
        word_emb = self.word_embed(x1)   # (batch_size, seq_len, word_embed_size)
        ...
        char_emb = char_emb.view(word_emb.size(0), -1, char_emb.size(1)) # (batch_size, seq_len, char_channel_size)

        emb = torch.cat([char_emb, word_emb], dim=-1) # (batch_size, seq_len, char_channel_size + word_embed_size)
...
class TransformerEncoder(nn.Module):
    def __init__(self, max_len, dim=8, output_dim= 100, num_layers=2, nhead=2, dropout=0.0, pad_idx = 0):
        super().__init__()
