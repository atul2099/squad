...
class BiDAF(nn.Module):
    ...
    def __init__(self, word_vectors, char_vectors, hidden_size, drop_prob=0., char_channel_size=100, char_channel_width=4):
        ...
        self.enc = layers.TransformerEncoder(max_len = 400, dim = hidden_size, num_layers = 2 ,nhead=4,  dropout = drop_prob)
...
#         self.mod = layers.TransformerEncoder(max_len = 400, dim = 2*hidden_size, output_dim = hidden_size//2,num_layers = 2,nhead=4, dropout = drop_prob)
    ...
    def forward(self, cw_idxs, cc_idxs, qw_idxs, qc_idxs):
        ...
        c_enc = self.enc(c_emb, cw_idxs)    # (batch_size, c_len, 2 * hidden_size)
        q_enc = self.enc(q_emb, qw_idxs)    # (batch_size, q_len, 2 * hidden_size)

        att = self.att(c_enc, q_enc,
                       c_mask, q_mask)    # (batch_size, c_len, 8 * hidden_size)

        mod = self.mod(att, cw_idxs)        # (batch_size, c_len, 2 * hidden_size)
